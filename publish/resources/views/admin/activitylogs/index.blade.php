                                ...
                                <table class="table table-striped table-bordered zero-configuration data-table w-100">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="align-middle text-center">{{ trans('activity logs.activity') }}
                                            </th>
                                            ...
                                            <th class="align-middle text-center" style="width: 25%;">
                                                @lang('general.action')</th>
                                        </tr>
                                    <tbody>
                                        @foreach($activitylogs as $item)
                                        <tr>
                                            <td class="align-middle text-center">
                                                <input type="checkbox" id="record-{{$item->id}}" value="{{$item->id}}"
                                            ...
                                            <td class="align-middle text-center">
                                                @if ($item->causer)
                                                <a href="{{ url('/admin/users/' . $item->causer->id) }}">{{
                                                    $item->causer->name }}</a>
                                                @else
                                                -
                                                @endif
                                            ...
                                            <td class="align-middle text-center">
                                                <a href="{{route('admin.activitylogs.show', $item->id)}}"
                                                    class="btn btn-jinja btn-sm ml-1 mr-1"><i class="fa fa-eye"></i>
                                                </a>
                                                <form action="{{route('admin.activitylogs.destroy', $item->id)}}"
                                                    class="my-1 my-xl-0" method="post" style="display: inline-block;">
                                                    @method('DELETE')
                                                    @csrf
                                                    <button type="submit" class="btn btn-jinja btn-sm delete"><i
